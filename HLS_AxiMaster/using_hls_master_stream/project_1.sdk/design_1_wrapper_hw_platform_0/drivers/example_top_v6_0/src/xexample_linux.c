...
int XExample_Initialize(XExample *InstancePtr, const char* InstanceName) {
    ...
    // NOTE: slave interface 'Myaxil' should be mapped to uioX/map0
    InstancePtr->Myaxil_BaseAddress = (u32)mmap(NULL, InfoPtr->maps[0].size, PROT_READ|PROT_WRITE, MAP_SHARED, InfoPtr->uio_fd, 0 * getpagesize());
    assert(InstancePtr->Myaxil_BaseAddress);
...
int XExample_Release(XExample *InstancePtr) {
    ...
    munmap((void*)InstancePtr->Myaxil_BaseAddress, InfoPtr->maps[0].size);
